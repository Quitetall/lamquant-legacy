...
def test_compress_batch_lossless_mirrors_tree(corpus: Path, tmp_path: Path):
    ...
    for p in expected:
        assert p.exists()
        raw = p.read_bytes()
        # LML file starts with 'LML' (container magic or ASCII prefix)
        assert raw[:3] == b'LML', f"Bad magic: {raw[:4]!r}"
...
def test_verify_batch_passes_clean_files(corpus: Path, tmp_path: Path):
    ...
    for r in rep.results:
        assert 'ok' in r.output_path  # LML or LML ok
...
def test_verify_batch_catches_truncated_file(tmp_path: Path):
    ...
    rep = verify_batch(inputs=[str(out)], decode=True, recursive=True,
                       workers=1, quiet=True)
    assert rep.n_failed == 1
    assert any('truncat' in (r.error or '').lower() or
               'exceeds' in (r.error or '').lower() or
               'incomplete' in (r.error or '').lower() or
               'corrupt' in (r.error or '').lower()
               for r in rep.results)
