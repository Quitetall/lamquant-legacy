...
def test_magic_is_lmq1():
    """compress.py and decompress.py agree on the magic bytes."""
    assert MAGIC_COMPRESS == b'LMQ1'
    assert MAGIC_DECOMPRESS == b'LMQ1'


def test_lmq2_is_rejected_clearly():
    """Old broken format must fail with a helpful error, not silent garbage."""
    fake_lmq2 = b'LMQ2' + b'\x00' * 100
    with pytest.raises(ValueError, match='newer|version'):
        _decompress_bytes(fake_lmq2)
